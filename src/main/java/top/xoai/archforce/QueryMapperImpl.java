...
{
    private final Logger logger = LoggerFactory.getLogger(QueryMapperImpl.class);

    @Autowired
    private JdbcTemplate jdbcTemplate;

    @Override
    public CountObject countIncrease(Long baseIndex) {
        String sql = "select count(1) increasedCount,sum(case type when 1 then 1 else 0 end) as indicator1," +
                ...
                "sum(case type when 9 then 1 else 0 end) as indicator9 from (select * from item order by item_id limit " + baseIndex + ",9999999999) t";
//        logger.info(sql);
